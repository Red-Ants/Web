    ...
    class DBConnect{
        ...
        function connect(){
           ...
           if($this->serverName == "redants.info" || $this->serverName == "www.redants.info" ){
            ...
            else if($this->serverName == "192.168.100.5"){
                $this->serverName = "192.168.100.5";
                $this->username = "admin";
                $this->password = "12345";
                $this->dbName = "red_ants";
                $this->conn = new mysqli($this->serverName, $this->username, $this->password, $this->dbName);
                    if($this->conn->connect_error){
                        echo "<script>console.log('Connection Failed with 192.168.100.5.')</script>";
                    }
                    else{
                        echo "<script>console.log('Connection Successful with 192.168.100.5.')</script>";
                    }
            }
